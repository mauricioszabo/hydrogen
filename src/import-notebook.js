...
function ipynbOpener(uri) {
    if (path.extname(uri).toLowerCase() === ".ipynb" &&
        ...
        return _loadNotebook(uri, atom.config.get("hydron.importNotebookResults"));
    }
}
...
function importNotebook(event) {
    ...
    if (filenameFromTreeView && path.extname(filenameFromTreeView) === ".ipynb") {
        return _loadNotebook(filenameFromTreeView, atom.config.get("hydron.importNotebookResults"));
    }
    dialog.showOpenDialog({
        ...
        _loadNotebook(filename, atom.config.get("hydron.importNotebookResults"));
    });
}
...
function toHydronCodeBlock(cell, commentStartString) {
